...
public class TestCancelBooking {
	...
	public void testCancelBooking() {
		Date from = getDate(2016, 5, 11);
		Date to = getDate(2016, 5, 18);
		cancelBooking(from, to);
		assertEquals(3, bookings.size());
	}
	
	private boolean cancelBooking(Date from, Date to) {
		boolean isBooked = false;
		int index = 0;
		for (Booking booking : bookings) {
			isBooked = booking.getBookingPeriod().getFrom().after(from) && booking.getBookingPeriod().getTo().before(to);
			index++;
			if (isBooked) {
